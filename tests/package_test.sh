...
"$PACK" "$ROOT/games/showcase_wick" "$WORK/lantnight.lant"
...
SHOWCASE_AUTO=1 LANTERN_FIXED_DT=1 LANTERN_SHOT="$WORK/packed" \
    "$LANTERN" "$WORK/lantnight.lant" >/dev/null 2>&1
cmp "$WORK/folder.bmp" "$WORK/packed.bmp"
echo "byte-identical"

echo "== corrupt one byte: runtime must refuse =="
SIZE=$(stat -f %z "$WORK/lantnight.lant" 2>/dev/null || stat -c %s "$WORK/lantnight.lant")
MID=$((SIZE / 2))
printf '\xFF' | dd of="$WORK/lantnight.lant" bs=1 seek="$MID" conv=notrunc 2>/dev/null
if "$LANTERN" "$WORK/lantnight.lant" >/dev/null 2>"$WORK/err.txt"; then
